...
final class Analytics_Tests: XCTestCase {
    ...
    func testDestinationInitialUpdateOnlyOnce() {
        ...
        UserDefaults.standard.removePersistentDomain(forName: "com.krenalis.storage.test")
    ...
    func testDestinationMetadata() {
        ...
        // we want the output reader on the krenalis plugin
        // cuz that's the only place the metadata is getting added.
        let krenalisDest = analytics.find(pluginType: KrenalisDestination.self)
        krenalisDest?.add(plugin: outputReader)
    ...
    func testEnrichment() {
        ...
        krenalis?.add(enrichment: destEnrichment)
    ...
    // Linux doesn't know what URLProtocol is and on watchOS it somehow works differently and isn't hit.
        ...
        func testFailedKrenalisResponse() throws {
            ...
            krenalis?.httpClient?.session = blockSession
    ...
    func testAnonIDGenerator() throws {
        ...
        // need to clear settings for this one.
        UserDefaults.standard.removePersistentDomain(forName: "com.krenalis.storage.anonIdGenerator")
