...
class StorageTests: XCTestCase {
    ...
    func testEventWriting() throws {
        ...
        XCTAssertTrue(fileURL.lastPathComponent == "0-krenalis-events.temp")
    ...
    func testFilePrepAndFinish() {
        ...
        XCTAssertTrue(fileURL.isFileURL)
        XCTAssertTrue(fileURL.lastPathComponent == "1-krenalis-events.temp")
        XCTAssertTrue(FileManager.default.fileExists(atPath: fileURL.path))
    }
    ...
    func testMigrationFromOldLocation() {
        ...
        try? fm.removeItem(at: newKrenalisDir)
        ...
        let oldKrenalisDir = oldBaseURL.appendingPathComponent("krenalis/\(writeKey)")
        try! fm.createDirectory(at: oldKrenalisDir, withIntermediateDirectories: true, attributes: nil)
        
        // Write some fake event files
        let testFile1 = oldKrenalisDir.appendingPathComponent("0-krenalis-events.temp")
        let testFile2 = oldKrenalisDir.appendingPathComponent("1-krenalis-events.temp")
        ...
        XCTAssertTrue(fm.fileExists(atPath: resultURL.appendingPathComponent("0-krenalis-events.temp").path))
        XCTAssertTrue(fm.fileExists(atPath: resultURL.appendingPathComponent("1-krenalis-events.temp").path))
        
        // Verify old directory is gone
        XCTAssertFalse(fm.fileExists(atPath: oldKrenalisDir.path))
