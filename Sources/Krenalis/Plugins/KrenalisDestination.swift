...
public class KrenalisAnonymousId: AnonymousIdGenerator {
    public func newAnonymousId() -> String {
        return UUID().uuidString
    }
}

public class KrenalisDestination: DestinationPlugin, Subscriber, FlushCompletion {
    internal enum Constants: String {
        case integrationName = "Krenalis"
    ...
    private let uploadsQueue = DispatchQueue(label: "uploadsQueue.krenalis.com")
    ...
    public func update(settings: Settings, type: UpdateType) {
        ...
        let krenalisInfo = settings.integrationSettings(forKey: self.key)
        // if customer specifies a different endpoint in Krenalis
        if let endpoint = krenalisInfo?[Self.Constants.endpoint.rawValue] as? String, endpoint.isEmpty == false {
            ...
            for url in files {
                ...
                autoreleasepool {
                    ...
                    let uploadTask = httpClient.startBatchUpload(writeKey: analytics.configuration.values.writeKey, batch: url) { [weak self] result in
                        ...
                        case .failure(Krenalis.HTTPClientErrors.statusCode(code: 400)):
                            storage.remove(data: [url])
                            cleanupUploads()
                        default:
                            break
...
extension KrenalisDestination: VersionedPlugin {
    public static func version() -> String {
        return __krenalis_version
    }
}
