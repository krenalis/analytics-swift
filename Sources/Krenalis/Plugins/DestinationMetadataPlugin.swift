...
public class DestinationMetadataPlugin: Plugin {
    ...
    public func execute<T: RawEvent>(event: T?) -> T? {
        ...
        for plugin in destinations {
            // Skip processing for krenalis
            if (plugin is KrenalisDestination) {
                continue
            }
        ...
        for (integration, _) in activeIntegrations {
            if (integration != "Krenalis" && !bundled.contains(integration)) {
                unbundled.insert(integration)
            }
        }

        let krenalisInfo = integrationSettings.integrationSettings(forKey: "Krenalis")
        let unbundledIntegrations = krenalisInfo?["unbundledIntegrations"] as? [String] ?? []
