...
private var __krenalis_connectionStatus: ConnectionStatus = .unknown
private var __krenalis_connectionStatusTimer: QueueTimer? = nil
private var __krenalis_connectionStatusLock = NSLock()

internal func __krenalis_connectionStatusCheck() -> ConnectionStatus {
    var zeroAddress = sockaddr_in()
    zeroAddress.sin_len = UInt8(MemoryLayout.size(ofValue: zeroAddress))
    zeroAddress.sin_family = sa_family_t(AF_INET)
...
internal func connectionStatus() -> ConnectionStatus {
    ...
    if __krenalis_connectionStatusTimer == nil {
        __krenalis_connectionStatusTimer = QueueTimer(interval: 300, immediate: true) {
            __krenalis_connectionStatusLock.lock()
            defer { __krenalis_connectionStatusLock.unlock() }
            __krenalis_connectionStatus = __krenalis_connectionStatusCheck()
        }
    }
    
    __krenalis_connectionStatusLock.lock()
    defer { __krenalis_connectionStatusLock.unlock() }
    return __krenalis_connectionStatus
}
*/
#endif
