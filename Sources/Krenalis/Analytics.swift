...
extension Analytics {
    /// Determine if there are any events that have yet to be sent to Krenalis
    public var hasUnsentEvents: Bool {
        if let krenalisDest = self.find(pluginType: KrenalisDestination.self) {
            if krenalisDest.pendingUploads > 0 {
                return true
            }
            if krenalisDest.eventCount > 0 {
                return true
            }
        }
