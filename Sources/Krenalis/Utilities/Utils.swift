...
internal func eventStorageDirectory(writeKey: String) -> URL {
    ...
    let krenalisURL = appSupportURL.appendingPathComponent("krenalis/\(writeKey)/")
    
    // Handle one-time migration from old locations
    migrateFromOldLocations(writeKey: writeKey, to: krenalisURL)
    
    // try to create it, will fail if already exists, nbd.
    // tvOS, watchOS regularly clear out data.
    try? FileManager.default.createDirectory(at: krenalisURL, withIntermediateDirectories: true, attributes: nil)
    return krenalisURL
}
...
private func migrateFromOldLocations(writeKey: String, to newLocation: URL) {
    ...
    // Get the parent of where our new krenalis directory should live
    let appSupportURL = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
    let newKrenalisDir = appSupportURL.appendingPathComponent("krenalis")
    
    // If krenalis dir already exists in app support, we're done
    guard !fm.fileExists(atPath: newKrenalisDir.path) else { return }
    ...
    let oldKrenalisDir = oldBaseURL.appendingPathComponent("krenalis")
    
    guard fm.fileExists(atPath: oldKrenalisDir.path) else { return }
    
    do {
        try fm.moveItem(at: oldKrenalisDir, to: newKrenalisDir)
        Analytics.krenalisLog(message: "Migrated analytics data from \(oldKrenalisDir.path)", kind: .debug)
    } catch {
        Analytics.krenalisLog(message: "Failed to migrate from \(oldKrenalisDir.path): \(error)", kind: .error)
    }
}
