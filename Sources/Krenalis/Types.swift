...
extension RawEvent {
    ...
    public mutating func disableCloudIntegrations(exceptKeys: [String]? = nil) {
        guard let existing = integrations?.dictionaryValue else {
            ...
            Analytics.krenalisLog(message: "Unable to get what should be a valid list of integrations from event.", kind: .error)
            return
        }
        var new = [String: Any]()
        ...
        do {
            ...
            Analytics.krenalisLog(message: "Unable to convert list of integrations to JSON. \(error)", kind: .error)
        }
    }
